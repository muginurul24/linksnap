...
- [ ] Install ALL dependencies listed in Quick Start
- [ ] Configure NativeWind with premium color palette in tailwind.config.ts
- [ ] Load Inter font (400, 500, 600, 700, 800) via `expo-font` in root _layout.tsx
- [ ] Configure Expo Router with dark theme
- [ ] Set StatusBar to `style="light"` globally
- [ ] Verify: `rtk bun run ios` or `rtk bun run android` starts with dark splash

### M1.2 — Design System Foundation
- [ ] `src/lib/constants/theme.ts` — export color palette, spacing, typography as constants
- [ ] `tailwind.config.ts` — add ALL colors from premium palette (surface, accent, content, glass)
- [ ] Create base UI components following design system:
...
- [ ] Every component MUST use the premium design patterns from the Design System section above
...
- [ ] `src/lib/api/auth.ts` — login, register, verifyEmail, forgotPassword, resetPassword
- [ ] `src/lib/api/links.ts` — CRUD links, link pages, smart rules
- [ ] `src/lib/api/analytics.ts` — link analytics, dashboard overview
- [ ] `src/lib/api/payments.ts` — create payment, payment history, subscription status, checkout
- [ ] `src/lib/api/settings.ts` — profile, security, API keys, notifications
- [ ] `src/lib/api/dashboard.ts` — overview stats (links count, clicks today, active campaigns)

### M1.4 — Auth Store & Secure Storage
- [ ] `src/lib/stores/auth-store.ts` (Zustand + persist)
...
- [ ] `src/lib/stores/app-store.ts` (Zustand)
...
- [ ] `src/providers/index.tsx` — combined providers component
...
- [ ] Design: Dark background, centered card with glassmorphism
- [ ] Logo/wordmark at top (LinkSnap in gold), tagline below
- [ ] Email + Password inputs with icons (Mail, Lock from lucide-react-native)
- [ ] "Forgot password?" link (accent color, right-aligned)
- [ ] Primary button: "Sign In" (gold accent, full width, haptic on press)
- [ ] Divider: "or continue with" with horizontal lines
- [ ] Google sign-in button (white outline on dark)
- [ ] Bottom link: "Don't have an account? Sign up" (accent)
- [ ] Keyboard-aware scroll view (react-native-keyboard-aware-scroll-view or KeyboardAvoidingView)
- [ ] Loading state: button shows spinner, disabled
- [ ] Error state: inline error message below inputs (red, with shake animation)
- [ ] Success: navigate to verify if email unverified, else to tabs

### M2.2 — Register Screen
- [ ] File: `app/(auth)/register.tsx`
- [ ] Name, Email, Password, Confirm Password inputs
- [ ] Password strength indicator bar (3 segments: weak/medium/strong, colors: red/yellow/green)
- [ ] Password requirements checklist (min 8 chars, uppercase, number, special char)
- [ ] Terms checkbox (required) with link to Terms page
- [ ] Primary button: "Create Account" (gold accent)
...
- [ ] Envelope illustration (icon) at top
- [ ] "Check your email" heading
- [ ] "We sent a 6-digit code to {email}" body text
- [ ] 6 individual OTP input boxes (auto-advance focus, paste from clipboard support)
- [ ] Timer: "Resend code in {countdown}s" — countdown from 60s
- [ ] "Resend code" button (enabled after countdown, with haptic)
- [ ] On verify success: haptic success + auto-navigate to tabs after 500ms delay
- [ ] On verify error: shake animation on OTP boxes + error message

### M2.4 — Auth Layout & Navigation
- [ ] File: `app/(auth)/_layout.tsx`
- [ ] Stack navigator: Login → Register → Verify
- [ ] Screen options: headerShown: false (all custom UI)
- [ ] Check auth on mount: if valid token → redirect to tabs
- [ ] Deep link: `linksnap://verify?email=...&token=...` → auto-fill OTP
- [ ] Back gesture enabled (swipe from left edge)

### M2.5 — Auth Security
- [ ] Token in SecureStore (iOS Keychain / Android Keystore), NEVER AsyncStorage
- [ ] Biometric unlock: prompt on app foreground with Face ID / fingerprint
  - Store `isBiometricEnabled` pref in SecureStore
  - Fallback to passcode if biometric fails 3 times
- [ ] Session timeout: auto-logout after 7 days of inactivity (check token iat claim)
- [ ] Certificate pinning via Expo plugin (prevent MITM)
- [ ] No token logged to console — strip via babel plugin in production
...
- [ ] **Header:** Greeting "Good morning, {name} 👋" + date, profile avatar (top right, tappable → settings)
- [ ] **Stats Row:** 3 glass cards in horizontal ScrollView
...
- [ ] **Quick Actions:** 2x2 grid of action cards (gold accent on hover)
  - "Create Link" (Plus icon), "Scan QR" (QrCode icon)
  - "My Links" (Link icon), "Campaigns" (Target icon)
- [ ] **Recent Links:** Section header "Recent Links" + "View All →"
  - 3-5 LinkRow components (glass card style)
  - Each row: favicon/slug, destination preview (1 line), clicks today badge, time ago
- [ ] **Subscription Banner** (if on FREE plan): glass card with gold accent
  - "Upgrade to Pro" with feature preview, tappable → billing screen
- [ ] Refetch on focus (useIsFocused or useFocusEffect)
- [ ] Pull-to-refresh
- [ ] Skeleton loading on first load

### M3.2 — My Links Screen
- [ ] File: `app/(tabs)/links.tsx`
- [ ] **Header:** "My Links" + filter/sort button (SlidersHorizontal icon)
- [ ] **Search bar:** glass input with Search icon, debounced 300ms
- [ ] **Filter chips:** horizontal scroll — All, Active, With Pages, By Campaign
  - Each chip: pill shape, glass border, active = accent bg + dark text
- [ ] **FlatList:** paginated, infinite scroll (onEndReached)
...
- [ ] **Sort sheet:** bottom sheet modal with sort options
  - Newest, Most Clicked, Alphabetical — radio selection
  - Apply button at bottom
- [ ] Empty state: illustration + "Create your first link" CTA
- [ ] Skeleton rows on first load (3-5 ghost cards)

### M3.3 — Quick Create Screen
- [ ] File: `app/(tabs)/create.tsx`
- [ ] This is the CENTER tab (FAB-style, visually elevated)
- [ ] **Top section:** Large URL input with "Paste" button (reads clipboard)
  - Paste button with clipboard icon, haptic on tap
  - URL validation: show green checkmark if valid
- [ ] **Short link preview:** generated slug displayed in glass card
  - "linksnap.id/{slug}" with Copy button
  - Animated checkmark on copy
- [ ] **Optional fields:** expandable section (collapsible)
  - Custom slug input
  - Title input
  - Enable Link Page toggle (Switch component, accent color)
- [ ] **Create button:** full-width gold accent, "Shorten & Share"
  - Loading spinner on press
  - Success: show share sheet immediately (expo-sharing or Share API)
  - Haptic success feedback
- [ ] **Recent links:** below create section, last 3 created links with copy action
- [ ] Keyboard dismiss on scroll

### M3.4 — Link Detail Screen
- [ ] File: `app/link/[id].tsx`
- [ ] **Header:** Back button + "Link Details" + Edit button (Pencil icon, top right)
- [ ] **URL Card:** big glass card showing short URL prominently
  - "linksnap.id/{slug}" in text-h1, Copy button, Share button
  - Animated checkmark on copy
- [ ] **Stats Row:** 3 stat cards — Total Clicks, Clicks Today, Unique Visitors
- [ ] **Destination:** glass card showing original URL (truncated, tappable to open)
- [ ] **Quick Actions Grid:** 
...
- [ ] **Link Page Card:** if enabled — preview with brand name, CTA, stats
- [ ] **Smart Rules Card:** if any — list rules with condition chips
- [ ] Delete confirmation: bottom sheet with red "Delete Link" button, cancel option
- [ ] Animated header: slug text scales down on scroll (if using ScrollView with Reanimated)
...
- [ ] **Basic Info section:** glass card
  ...
  - Title input
- [ ] **Link Page section:** toggle + expandable glass card
  - Brand/logo name
...
- [ ] **Smart Rules section:** toggle + expandable
...
- [ ] **Save button:** gold accent, full width, sticky at bottom
...
- [ ] **Header:** Back + "Analytics" + date range picker (top right)
- [ ] **Date range chips:** horizontal scroll — 7D, 30D, 90D, All Time
  - Active chip: accent bg + dark text
- [ ] **Clicks Chart:** line chart (custom SVG or victory-native)
...
- [ ] **Stats Grid:** 2x2 grid of glass stat cards
  - Total Clicks, Unique Visitors, Avg CTR, Bounce Rate
  - Each: icon + value (text-display) + label (text-caption) + % change badge
- [ ] **Top Countries:** ranked list with flag emoji + country name + count + progress bar
- [ ] **Device Breakdown:** horizontal bar chart or 3 cards
  - Mobile %, Desktop %, Tablet % — with icons
- [ ] **Top Referrers:** list with source name + count
- [ ] **Export button:** Share CSV (generate + share via native sheet)
- [ ] Empty state if no click data: "No clicks yet — share your link to get started"
- [ ] Full dark charts — gridlines in surface-300, data in accent gradient

### M3.7 — Campaigns Screen
- [ ] File: `app/(tabs)/campaigns.tsx` (or accessed from dashboard)
- [ ] **Header:** "Campaigns" + Create button (Plus icon)
- [ ] **Campaign cards:** glass card, tappable → campaign detail
...
- [ ] **Current Plan Card:** accent glass card at top
...
- [ ] **Plan Comparison:** 3 plan cards in horizontal ScrollView (or stacked on small screens)
...
- [ ] **Toggle:** Monthly / Yearly (segmented control, 20% discount shown on yearly)
- [ ] **Billing History:** section "Billing History" 
  - List of past transactions: date, amount, status badge (Paid/Pending/Failed)
  - Each row: tappable → invoice detail (if available)
- [ ] **Cancel Subscription:** text link at bottom (if active subscription)
  - Confirmation sheet with reason picker
- [ ] Close button (X icon, top left) or back navigation

### M3.9 — Settings & Profile Screen
- [ ] File: `app/(tabs)/settings.tsx`
- [ ] **Profile Header:** avatar (large, 72px, gold ring), name, email, plan badge
- [ ] **Settings Sections** (glass cards with section dividers):
...
- [ ] **Profile Edit:** `app/settings/profile.tsx`
...
- [ ] **API Keys:** `app/settings/api-keys.tsx`
...
- [ ] **Logout button:** ghost style at very bottom, red text
...
- [ ] Camera permission request flow with explanation
- [ ] Full-screen scanner with overlay guide (rounded corner frame, accent border)
- [ ] On scan: haptic feedback → if it's a valid URL, show preview sheet
  - "Open URL" and "Create Short Link" actions
  - If it's a linksnap.id link, navigate to link detail
- [ ] Flashlight toggle button (bottom)
- [ ] Close button (X icon, top left)
- [ ] Error states: camera permission denied, unsupported device
- [ ] Accessible from: Quick Create screen, tab bar scan action, dashboard quick action

### M4.2 — Push Notifications
- [ ] File: `src/lib/hooks/useNotifications.ts`
- [ ] Request permission on first login (after onboarding)
- [ ] Register push token via Expo Notifications
- [ ] Send token to backend: `POST /api/v1/settings/devices`
...
- [ ] Notification types + deep link mappings:
...
- [ ] Receive shared URLs from other apps (system share sheet)
- [ ] Configure `app.json` for share extension (iOS) + intent filter (Android)
- [ ] Flow: receive URL → app opens to create screen with URL pre-filled
- [ ] Use Expo Linking: `linksnap://create?url={encoded_url}`
- [ ] Deep link handler in root _layout.tsx

### M4.4 — Offline Mode
- [ ] Queue mutations while offline using `@tanstack/react-query` persist
- [ ] Show "Offline" banner at top (glass, amber accent, WiFi Off icon)
- [ ] Pending actions badge on create button
- [ ] Auto-sync when back online (NetInfo listener via `@react-native-community/netinfo`)
- [ ] Cache recent links + dashboard stats for instant load
- [ ] Stale-while-revalidate pattern for all queries (show cache while refetching)
...
- [ ] Skeleton loaders matching exact layout of each screen (not generic rectangles)
...
- [ ] Error states: glass card with error icon + message + "Retry" button
- [ ] Offline banner: glass card with amber accent, fixed at top, animated slide-in

### M5.2 — Animations & Micro-interactions
- [ ] Screen transitions: Reanimated FadeIn (300ms) on every screen
- [ ] List items: FadeInUp with stagger (50ms delay per item, spring physics)
- [ ] Tab bar: smooth indicator slide (Reanimated shared value, spring config)
- [ ] Pull-to-refresh: custom spinner with gold accent color
- [ ] Button press: scale 0.97 + 100ms spring back + haptic light
- [ ] Copy to clipboard: animated checkmark (scale from 0 to 1, spring)
- [ ] Success actions: subtle gold pulse/glow on the affected element
- [ ] Delete: item shrinks + fades out (layout animation)
- [ ] Number count-up animation on dashboard stats (animate from 0 to value)
- [ ] Skeleton shimmer: moving gradient at 45° angle
...
- [ ] VoiceOver: logical focus order, meaningful labels
- [ ] Color contrast: WCAG AA minimum (4.5:1 for text, 3:1 for large text)
- [ ] Reduce Motion: respect `accessibilityReduceMotion` — disable spring animations
- [ ] Haptic feedback respects system accessibility setting

### M5.4 — Performance
- [ ] Hermes engine enabled in production
- [ ] FlatList with `getItemLayout` for fixed-height rows
- [ ] `React.memo` on LinkRow, CampaignCard, StatsCard
- [ ] Image caching via `expo-image` (fast progressive loading)
- [ ] List empty/loading handled with `ListEmptyComponent` / `ListFooterComponent`
- [ ] Avoid inline functions in renderItem
- [ ] Bundle size: keep JS bundle under 2MB (analyze with `expo-export`)

### M5.5 — EAS Build & Submit
- [ ] `eas.json` configured: development, preview, production profiles
- [ ] iOS: App Store Connect (certificates, provisioning profiles via EAS)
- [ ] Android: Google Play Console (keystore managed by EAS)
- [ ] App icon: premium gold-on-black icon with LinkSnap logo
- [ ] Splash screen: dark background with gold logo, fade-out animation (500ms)
- [ ] App Store metadata: title, subtitle, description, keywords, screenshots (6.7" + 5.5")
- [ ] TestFlight internal testing build → distribute to testers
...
- [ ] **No sensitive data in logs** — Strip tokens from console.log via babel plugin in production
...
- [ ] **Jailbreak/Root detection** — `expo-device` `isDevice` check + jailbreak detection package
- [ ] **App Transport Security** — iOS ATS enforced via Info.plist
- [ ] **ProGuard/R8** — Android code obfuscation enabled
- [ ] **Hermes engine** — JS engine hardened for production
- [ ] **API calls server-to-server for sensitive ops** — payment creation, webhooks stay server-side
- [ ] **Biometric fallback** — Max 3 attempts before requiring passcode
