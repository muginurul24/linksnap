...
export default function AdminAuditLogError({
  ...
  useEffect(() => {
    logger.error("admin_audit_log_error_boundary", {
      digest: error.digest,
      name: error.name,
  ...
  return (
    <div className="flex flex-col items-center gap-4 rounded-lg border bg-card p-8 text-center">
      ...
      <div>
        <p className="text-sm font-medium">Audit log is temporarily unavailable</p>
        <p className="mt-1 text-sm text-muted-foreground">
          An error occurred while loading the audit trail. This might be a temporary database issue.
        </p>
      </div>
